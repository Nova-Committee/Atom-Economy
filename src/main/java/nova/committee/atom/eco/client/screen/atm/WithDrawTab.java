 ...
 * Version: 1.0
 */
public class WithDrawTab extends ATMTab {
    ATMMenu atmMenu;
    Button buttonWithdraw;
    private EditBox values;

    protected WithDrawTab(ATMScreen screen) {
        super(screen);
    ...
    public void init() {
        this.values = this.screen.addRenderableTabWidget(
                new EditBox(screen.getFont(), screen.getGuiLeft() + 27, screen.getGuiTop() + 157, 105, 15, new TranslatableComponent("gui.atomeco.withdraw.withdraw_value")));

        this.buttonWithdraw = this.screen.addRenderableTabWidget(
    ...
    private void withdrawAct() {
        ...
        try {
            ...
            if (money > 0 && atmMenu.processSelfAction(money, false)) {
                Static.LOGGER.info("WithDraw success");
            }
        } catch (Exception e) {
    ...
    public void tick() {
        if (this.atmMenu.account == null) {
            ...
            this.buttonWithdraw.active = this.values.getValue() != null;
        }
    }
    ...
    public Component getTooltip() {
        return new TranslatableComponent("gui.atomeco.index.withdraw");
    }
}
